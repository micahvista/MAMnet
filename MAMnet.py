...
#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\tFORMAT\n"""
    ...
    for rec in pd.DataFrame(svlist).sort_values([0, 1]).values:
        ...
        geno = rec[4]
        if(rec[2]<0):
            recinfo = 'SVLEN=' + str(int(rec[2]))+';SVTYPE=' + 'DEL'+';END='+str(int(rec[1])+abs(int(rec[2])))+';RE='+str(rec[3])+'\tGT\t'+geno+'\n'
            myvcf += (contig +'\t'+ str(int(rec[1]))+'\t'+ '.'+'\t'+ '.'+'\t'+ 'A'+'\t'+ '.'+'\t'+ 'PASS'+'\t'+recinfo)
        elif(rec[2]>0):
            recinfo = 'SVLEN=' + str(int(rec[2]))+';SVTYPE=' + 'INS'+';END='+str(int(rec[1])+1)+';RE='+str(rec[3])+'\tGT\t'+geno+'\n'
            myvcf += (contig +'\t'+ str(int(rec[1]))+'\t'+ '.'+'\t'+ '.'+'\t'+ 'A'+'\t'+ '.'+'\t'+ 'PASS'+'\t'+recinfo)
